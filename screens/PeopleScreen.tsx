...
import { Persona, SuperCategory, PersonaRole } from '../types';
import { CrownIcon, AppLogo, SearchIcon, WalletIcon, GearIcon, UsersIcon } from '../components/icons/Icons';
...
const PersonaListItem: React.FC<PersonaListItemProps> = ({ persona, onSelect, isSelected }) => {
  ...
  return (
    <div
      onClick={handleClick}
      className={`flex items-start p-3 space-x-4 cursor-pointer transition-all bg-white dark:bg-gray-800 border rounded-xl shadow-sm hover:shadow-lg hover:border-gray-300 dark:hover:border-gray-600 ${isSelected ? 'border-green-500 dark:border-green-400 ring-2 ring-green-500/20 dark:ring-green-400/20' : 'border-gray-200 dark:border-gray-700'}`}
      role="button"
...
type MasanaTab = 'Women' | 'Men';

const TABS: MasanaTab[] = ['Women', 'Men'];

const MasanaScreen: React.FC = () => {
  const { personas, setSelectedPersona, setAppState, selectedPersona, tokenBalance } = useContext(AppContext);
  const [searchQuery, setSearchQuery] = useState('');
  const [activeTab, setActiveTab] = useState<MasanaTab>(TABS[0]);
  ...
  const filteredPersonas = useMemo(() => {
    ...
    const activeGender = activeTab === 'Women' ? 'female' : 'male';

    let basePersonas = personas.filter(p => 
        // Fix: Use PersonaRole.CONVERSATIONALIST to correctly filter for 'Masana' personas.
        p.role === PersonaRole.CONVERSATIONALIST && p.gender === activeGender
    );
    ...
    <div className="h-screen w-full flex flex-col bg-gray-100 dark:bg-gray-900 overflow-hidden">
      <header className={`p-4 flex items-center justify-between gap-2 border-b border-gray-200 dark:border-gray-700 flex-shrink-0 bg-white dark:bg-gray-800 ${!isMobile ? 'h-[73px]' : ''}`}>
        <div className="flex items-center space-x-2">
            ...
            <h1 className="text-xl font-bold">Masana</h1>
        </div>
        {isMobile ? (
            <div className="flex items-center space-x-2">
                 <LanguageSwitcher />
      ...
      <div className="flex-shrink-0 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
        <nav className="flex space-x-1 px-2" aria-label="Masana Categories">
            {TABS.map((tab) => (
                <button
                    key={tab}
                    onClick={() => {
